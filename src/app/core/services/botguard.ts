...
export class BotguardService {
  ...
  private proxyStore = inject(ProxySettingsStore); // Inyectamos el store
  ...
  private async _initBotguard() {
    const settings = this.proxyStore.settings(); // Obtenemos los settings

    // FIX: Pasamos los settings a fetch (que ahora es fetchFunction)
    ...
    if (this.bgChallenge) {
      ...
      // FIX: Pasamos los settings a fetchFunction
      const integrityTokenResponse = await fetchFunction(buildURL('GenerateIT', true), settings, {
        method: 'POST',
        headers: {
          'content-type': 'application/json+protobuf',
          'x-goog-api-key': GOOG_API_KEY,
