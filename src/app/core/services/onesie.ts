...
// Tipos locales (copiados de tu archivo original)
type OnesieRequestArgs = {
  clientConfig: OnesieHotConfig;
  innertubeRequest: Record<string, any>; // Este es el tipo que causa TS4111
};
type PartHandler = (part: Part) => void;

@Injectable({ providedIn: 'root' })
export class OnesieService {
  // Inyectamos el store de settings
  private proxyStore = inject(ProxySettingsStore);

  /**
   * Prepara el request body para Onesie.
   */
  private async prepareOnesieRequest(args: OnesieRequestArgs) {
    ...
    // FIX: TS4111 - Usar corchetes para acceder a propiedades
    const headers = [
      { name: 'Content-Type', value: 'application/json' },
      { name: 'User-Agent', value: innertubeRequest['context'].client.userAgent },
      { name: 'X-Goog-Visitor-Id', value: innertubeRequest['context'].client.visitorData },
    ];
  ...
  /**
   * Esta es la función pública que llamarán otros servicios/componentes.
   */
  public async makePlayerRequest(args: OnesieRequestArgs) {
    const { baseUrl, body, encodedVideoId } = await this.prepareOnesieRequest(args);

    // Obtenemos los settings actuales del proxy
    const proxySettings = this.proxyStore.settings();

    let redirectorResponseUrl = localStorage.getItem(REDIRECTOR_STORAGE_KEY);

    if (!redirectorResponseUrl) {
      // Pasamos los settings del proxy a fetchFunction
    ...
    // Pasamos los settings del proxy a fetchFunction
    const response = await fetchFunction(url, proxySettings, {
      method: 'POST',
      body: body as any,
    });
