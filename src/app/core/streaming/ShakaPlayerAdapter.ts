...
// FIX: TS2307 - Cambiada la ruta de importación de '@/' a relativa
import {
  asMap,
  checkExtension,
...
// (El resto de la clase es idéntica, solo agregamos tipos al forEach)
// ...
...
export class ShakaPlayerAdapter implements SabrPlayerAdapter {
  ...
  private async request(
    ...
    try {
      ...
      const fetchFn =
        uri.includes('get_drm_license') && checkExtension() ? getInjectedProxyFunction() : fetch;
  ...
  // ... (resto de la clase ShakaPlayerAdapter sin cambios) ...
  ...
  // Definimos ShakaResponseArgs aquí ya que no es exportada
  private createShakaResponse(args: {
    uri: string;
    request: shaka.extern.Request;
    requestType: shaka.net.NetworkingEngine.RequestType;
    response: Response;
